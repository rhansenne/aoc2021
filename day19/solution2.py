    ...
    for p in range(len(path)):
        if path[p]==0:
            ...
            if (path[p+1],path[p]) in transforms:
                ...
                norm_coord = transform(pos,tr[1],tr[2])
                pos= (norm_coord[0]+tr[0][0],norm_coord[1]+tr[0][1],norm_coord[2]+tr[0][2])
            else:
                tr=transforms[(path[p],path[p+1])]
                norm_coord = transform_inv(pos,tr[1],tr[2])
                norm_tr=transform_inv(tr[0],tr[1],tr[2])
                pos= (norm_coord[0]-norm_tr[0],norm_coord[1]-norm_tr[1],norm_coord[2]-norm_tr[2])
max_mh=0
for i in range(1,len(positions)):
    for j in range(i+1,len(positions)):
        mh = abs(positions[i][0]-positions[j][0])+abs(positions[i][1]-positions[j][1])+abs(positions[i][2]-positions[j][2])
        if mh>max_mh:
            max_mh=mh
print(max_mh)