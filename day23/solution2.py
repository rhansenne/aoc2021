...
for lnr, line in enumerate(open('input2.txt', 'r').readlines()):
    if 1<lnr<6:
...
def completed(diag):
    if diag[1]==diag[2]==diag[3]==diag[4]==[0,1,2,3]:
        return True
    return False
...
def diag_to_tuple(diag):
    return (tuple(diag[0]),tuple(diag[1]),tuple(diag[2]),tuple(diag[3]),tuple(diag[4]))

def move_to_hallway(diag,row,col):
    energy_per_step=pow(10,diag[row][col])
    destcol_energy=[]
    energy_to_hallway=0
    if row==4 and col==diag[row][col]: #already in room
        return destcol_energy
    if row==3 and col==diag[row][col]==diag[4][col]: #already in room and below also already in room
        return destcol_energy
    if row==2 and col==diag[row][col]==diag[3][col]==diag[4][col]: #already in room and below also already in room
        return destcol_energy
    if row==1 and col==diag[row][col]==diag[2][col]==diag[3][col]==diag[4][col]: #already in room and below also already in room
        return destcol_energy

    if row==4 and diag[1][col]==diag[2][col]==diag[3][col]==-1:
        row=0
        col=col*2+2
        energy_to_hallway+=4*energy_per_step
    if row==3 and diag[1][col]==diag[2][col]==-1:
        row=0
        col=col*2+2
        energy_to_hallway+=3*energy_per_step
...
def move_to_room(diag,row,col):    
    ...
    if diag[1][row1_col]>=0: #room occupied
        return None
    for r in (2,3,4):
        if diag[r][row1_col]>=0 and diag[r][row1_col]!=row1_col: #room bottom occupied by other type
            return None
    energy_per_step=pow(10,diag[row][col])

    if row==4:
        if diag[1][col]>=0 or diag[2][col]>=0 or diag[3][col]>=0:
            return None
        row=0
        col=col*2+2
        energy_to_room=4*energy_per_step
    if row==3:
        if diag[1][col]>=0 or diag[2][col]>=0:
            return None
        row=0
        col=col*2+2
        energy_to_room=3*energy_per_step
    if row==2:
        if diag[1][col]>=0:
            return None
        row=0
    ...
    if diag[4][row1_col]==diag[3][row1_col]==diag[2][row1_col]==-1:
        energy_to_room+=4*energy_per_step
        return (4,energy_to_room)
    elif diag[3][row1_col]==diag[2][row1_col]==-1:
        energy_to_room+=3*energy_per_step
        return (3,energy_to_room)
    elif diag[2][row1_col]==-1:
        energy_to_room+=2*energy_per_step
...
def next_move(diag,energy,last_moved):
    ...
    if diag_tuple in diag_energy_cache:
        if  diag_energy_cache[diag_tuple]<energy:
            return
    else:
        diag_energy_cache[diag_tuple]=energy

    for i,row in enumerate(diag):
        for j,anthropod in enumerate(row):
            if anthropod>=0 and not last_moved==(i,j) and not (i==4 and j==anthropod) and not (i==3 and j==diag[4][j]==anthropod) and not (i==2 and j==diag[3][j]==diag[4][j]==anthropod) and not (i==1 and j==diag[2][j]==diag[3][j]==diag[4][j]==anthropod): #don't move if already in place
                ...
                if i>0 and in_hallway<6: #hallway not full                     
